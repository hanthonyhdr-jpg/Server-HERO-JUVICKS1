...
_CSS_STATIC = """
    ...
    [data-testid="stSidebar"] {
        background-color: rgba(0, 0, 0, 0.95) !important;   /* ← FUNDO SIDEBAR */
    ...
    [data-testid="stSidebar"] .stMarkdown p {
        color: #c0c8d8 !important;    /* ← TEXTO SIDEBAR */
    }

    /* 3C. Item ativo / selecionado na Sidebar */
    [data-testid="stSidebar"] [aria-selected="true"],
    [data-testid="stSidebar"] [aria-current="page"] {
        background: rgba(10, 255, 0, 0.12) !important;  /* ← FUNDO ITEM ATIVO SIDEBAR */
        border-left: 3px solid #0aff00 !important;       /* ← BORDA ITEM ATIVO SIDEBAR */
    ...
    [data-testid="stSidebar"] h3 {
        color: #ffffff !important;    /* ← TÍTULOS SIDEBAR */
    }
    ...
    div[data-testid="stMetric"] {
        background: linear-gradient(145deg, rgba(15,23,42,0.80), rgba(0,0,0,0.80)); /* ← FUNDO CARD */
    ...
    div[data-testid="stMetric"]:hover {
        ...
        border-color: #0aff00;        /* ← COR HOVER BORDA CARD MÉTRICA */
        box-shadow: 0 20px 25px -5px rgba(0,0,0,0.3);
    }

    [data-testid="stVerticalBlock"] > div > div > div[data-testid="stContainer"] {
        background: rgba(10,15,30,0.75);  /* ← FUNDO CONTAINER */
    ...
    [data-testid="stVerticalBlock"] > div > div > div[data-testid="stContainer"]:hover {
        border-color: rgba(10, 255, 0, 0.4);  /* ← HOVER BORDA CONTAINER */
    }
    ...
    .stButton > button {
        ...
        background: linear-gradient(to bottom right, #0aff00, #0aff00) !important;  /* ← COR BOTÃO */
        color: white !important;      /* ← TEXTO BOTÃO */
    }
    .stButton > button:hover {
        transform: scale(1.02) !important;
        box-shadow: 0 0 15px rgba(10, 255, 0, 0.4) !important;  /* ← GLOW HOVER BOTÃO */
    }
    ...
    .stTextarea textarea {
        ...
        background-color: rgba(0, 0, 0, 0.6) !important;    /* ← FUNDO INPUT */
        border: 1px solid rgba(255,255,255,0.1) !important;
        color: #f1f5f9 !important;   /* ← TEXTO INPUT */
    }
    .stTextInput > div > div > input:focus,
    .stTextarea textarea:focus {
        border-color: #0aff00 !important;  /* ← BORDA FOCUS INPUT */
        box-shadow: 0 0 0 2px rgba(10,255,0,0.15) !important;
    }
    ...
    ::-webkit-scrollbar-track { background: #000000; }          /* ← TRACK SCROLLBAR */
    ::-webkit-scrollbar-thumb { background: #1e293b; border-radius: 100px; }  /* ← THUMB SCROLLBAR */
    ::-webkit-scrollbar-thumb:hover { background: #0aff00; }   /* ← HOVER SCROLLBAR */
    ...
    div[data-baseweb="tab-highlight"] {
        background-color: #0aff00 !important;  /* ← SUBLINHADO TAB ATIVA */
    }

    /* ═══════════════════════════════════════
       10. TIPOGRAFIA GERAL (Conteúdo principal)
       ═══════════════════════════════════════ */
    p, .stMarkdown p { color: #c0c8d8; }          /* ← TEXTO SECUNDÁRIO */
    label { color: #c0c8d8 !important; }           /* ← LABELS */
...
def apply_modern_style(logo_url=""):
    """
    Tema gerado pelo Editor Visual em 08/03/2026 14:38
    Edite _CSS_STATIC acima — chaves simples funcionam normalmente.
    Seções identificadas por comentários ← para edição fácil.
    """
...
def get_signature_client_style():
    """Retorna o CSS para a página pública de assinatura do cliente."""
    return """
        <style>
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600&display=swap');
        ...
        .stAppDeployButton, .stDeployButton { display: none !important; }
        .client-header { background: linear-gradient(135deg, #0ea5e9, #6366f1); padding: 32px; border-radius: 20px; text-align: center; margin-bottom: 24px; box-shadow: 0 10px 25px -5px rgba(14,165,233,0.3); }
...
def get_login_style():
    ...
    return """
        ...
        .stApp { background: radial-gradient(circle at 20% 20%, #1e293b 0%, #020617 100%) !important; font-family: 'Inter', sans-serif !important; }
        .block-container { padding-top: 0 !important; max-width: 100% !important; }
        .login-card { background: rgba(30,41,59,0.4); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.1); border-radius: 24px; padding: 40px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); margin-top: 20px; }
        ...
        .stButton > button { background: linear-gradient(135deg, #0ea5e9 0%, #0284c7 100%) !important; color: white !important; border: none !important; border-radius: 12px !important; font-size: 16px !important; font-weight: 600 !important; padding: 14px !important; width: 100% !important; }
        [data-testid="stAlert"] { border-radius: 12px !important; background-color: rgba(15,23,42,0.8) !important; border: 1px solid rgba(255,255,255,0.05) !important; }
        .stAppDeployButton, .stDeployButton { display: none !important; }
        </style>
    """
...
def apply_license_style():
    ...
    st.markdown("""
        ...
        .lock-container { max-width: 500px; margin: 60px auto; padding: 40px; background: rgba(15,23,42,0.6); backdrop-filter: blur(20px); border-radius: 28px; border: 1px solid rgba(255,255,255,0.1); text-align: center; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); }
        ...
        .support-link { display: inline-block; margin-top: 24px; color: #38bdf8 !important; text-decoration: none !important; font-size: 14px; }
        .stAppDeployButton, .stDeployButton { display: none !important; }
        </style>
    """, unsafe_allow_html=True)
